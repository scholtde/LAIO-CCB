...
SELECTING_FEATURE, TYPING = map(chr, range(13, 15))
# Meta states
...
(SELF, GENDER, MALE, FEMALE, AGE, NAME, SURNAME, NATIONALITY, IDENTIFICATION, SA_ID, PASSPORT, START_OVER, FEATURES,
 CURRENT_FEATURE, CURRENT_LEVEL) = map(chr, range(17, 32))


# Helper
def _name_switcher(level):
    if level == NATIONALITY:
        return ('USA', 'SA', 'ZIMBABWE', 'BOTSWANA', 'NAMIBIA')
    elif level == IDENTIFICATION:
        return ('ID', 'PASSPORT')
...
def main():
    ...
    updater = Updater("1105813279:AAFsJ0vazDkf7lktEwb6RXhDMWEqH9-CwAA", use_context=True)
    ...
    add_member_conv = ConversationHandler(
        ...
        states={
            SELECTING_LEVEL: [CallbackQueryHandler(select_gender,
                                                   pattern='^{0}$|^{1}$'.format(str(NATIONALITY),
                                                                                str(IDENTIFICATION)))],
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            SELECTING_ACTION: [
                add_member_conv,
                CallbackQueryHandler(show_data, pattern='^' + str(SHOWING) + '$'),
                CallbackQueryHandler(adding_self, pattern='^' + str(EMERGENCY) + '$'),
